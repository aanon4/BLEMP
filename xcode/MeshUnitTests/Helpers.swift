...
class Helpers {
  ...
  class func setValue(inout source: Mesh_Node, target: Int, key: UInt16, version: Mesh_Version, value: Int) {
    ...
    withUnsafeMutablePointer(&data[0]) { dataPtr -> Void in
      ...
      result = Mesh_SetValueInternal(&source, Mesh_InternNodeId(&source, &targetId, 1), mkey, dataSpace, UInt8(data.count), 1, version, simChangeBits(&source))
      simSync(&source)
    }
    XCTAssertEqual(result.value, MESH_OK.value, "Mesh_SetValue fail")
  }
  
  class func getValue(inout source: Mesh_Node, target: Int, key: UInt16) -> Int {
    var result = MESH_OK
    var gdata: [UInt8] = [0]
    var mkey = Mesh_Key()
    mkey.key = key
    withUnsafeMutablePointer(&gdata[0]) { gdataPtr -> Void in
      ...
      result = Mesh_GetValue(&source, Mesh_InternNodeId(&source, &id, 1), mkey, gdataPtr, &count)
    }
